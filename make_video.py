...
def draw_env(ax, env, alpha=0.16):
    ...
    for o in env.obstacles:
        if isinstance(o, SphereObstacle):
            ...
            ax.add_collection3d(Poly3DCollection(
                ...
                edgecolor="#6B7683", linewidths=0.25, alpha=alpha))
...
def main():
    ...
    for q in range(a.queries):
        ...
        shots.append(dict(env=env, s=start, g=goal, L=L, R=R, fl=fl, fr=fr, idx=idx))
    ...
    def setup(k):
        ...
        ln = ([axl.plot([], [], [], lw=0.9, alpha=0.85)[0] for _ in range(a.n_samples)],
              [axr.plot([], [], [], lw=0.9, alpha=0.85)[0] for _ in range(a.n_samples)])
        for j in range(a.n_samples):
            ln[0][j].set_color(FREE if sh["fl"][j] else BLOCKED)
            ln[1][j].set_color(FREE if sh["fr"][j] else BLOCKED)
            ln[0][j].set_zorder(5 if sh["fl"][j] else 2)
            ln[1][j].set_zorder(5 if sh["fr"][j] else 2)
        state["shot"], state["lines"] = k, ln
