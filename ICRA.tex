...
\newcommand{\lipA}{The task of motion planning is equivariant to rigid-body transformations, but the field that the flow matching network learns is not. In the current literature, there are three techniques to restore equivariance: in the data, in the operator, or in the weights.}

\newcommand{\lipB}{A motion planning task names a start and a goal, and those two points already determine a canonical frame. No existing work measures what that frame is worth before any technique is applied, and none separates how equivariant a trained model became from how much that equivariance bought it.
...
That is the quantity we bound, and it is not the quantity, we find, that predicts
anything.

\section{Method}
In this section, we begin by formally defining the generative motion planning objective and the symmetry it carries (Sec.~\ref{sec:problem}). We then decompose SE(3) into the part a planning query determines and the part it does not, and state precisely what survives that reduction (Sec.~\ref{sec:decompose}). To establish that the remainder cannot be removed by the same route, we give two independent obstructions, one topological and one distributional (Sec.~\ref{sec:obstruction}). Finally, we enumerate the three places a symmetry can be imposed, in the training data, in the inference operator, and in the weights (Sec.~\ref{sec:threeways}), and define the non-equivariance residual that bounds what the second of them can remove.
...
We formulate motion planning in a workspace $\mathbb{R}^d$ ($d=3$ throughout,
$d=6$ for the rigid-body domain of Sec.~\ref{sec:results}). A task is defined by
the tuple $\mathcal{P} = (\svec, \gvec, \mathcal{O})$, where
$\svec, \gvec \in \mathbb{R}^d$ are the start and goal configurations and
$\mathcal{O}$ is a set of convex obstacles inducing a signed distance field
$d_{\mathcal{O}} : \mathbb{R}^d \to \mathbb{R}$. A trajectory is a waypoint
sequence $x = [p_1, \dots, p_N] \in \mathbb{R}^{N \times d}$, and the feasible
set of $\mathcal{P}$ is
\begin{equation}
  \mathcal{S}(\mathcal{P}) =
  \big\{\, x \;:\; p_1 = \svec,\; p_N = \gvec,\;
  d_{\mathcal{O}}(p_i) \ge \rho \;\; \forall i \,\big\},
  \label{eq:feasible}
\end{equation}
with $\rho$ the robot radius. The objective is to learn a generative model
...
transports a simple prior to $p_\theta(x \mid \mathcal{P})$. Nothing in that
objective enforces Eq.~\eqref{eq:planner-equivariance}.
...
Write $m = \tfrac{1}{2}(\svec + \gvec)$, $d = \|\gvec - \svec\|$
and $\hat{u} = (\gvec - \svec)/d$, and let $R_{\mathcal{P}} \in \mathrm{SO}(3)$
be the rotation whose rows are $(\hat{u},\, \hat{v},\, \hat{u} \times \hat{v})$
for some unit $\hat{v} \perp \hat{u}$. The reduction is the rigid map
\begin{equation}
  T_{\mathcal{P}}(p) = R_{\mathcal{P}}\,(p - m),
  \label{eq:reduction}
\end{equation}
under which the endpoints stop being inputs and become constants,
\begin{equation}
  T_{\mathcal{P}}(\svec) = \big({-\tfrac{d}{2}},\, 0,\, 0\big),
  \qquad
  T_{\mathcal{P}}(\gvec) = \big({+\tfrac{d}{2}},\, 0,\, 0\big).
  \label{eq:endpoints}
\end{equation}

The network never sees a world coordinate. The task is carried into the
frame, $\hat{x}_t = T_{\mathcal{P}}(x_t)$ and
$\hat{\mathcal{O}} = T_{\mathcal{P}}\mathcal{O}$, obstacle centres transforming
...
$\hat{v} = v_\theta(\hat{x}_t, t \mid d, \hat{\mathcal{O}})$, where by
Eq.~\eqref{eq:endpoints} the query enters as the single scalar $d$ in place of
...
through $\hat{u}$. It does not fix the sixth. The admissible $\hat{v}$ form a
circle in the plane normal to $\hat{u}$, and any rule selecting one of them must
read something beyond $\svec$ and $\gvec$; ours reads the world coordinate axis
least aligned with $\hat{u}$ and orthogonalises against it. That rule turns with
the world. Replacing $\mathcal{P}$ by $T\mathcal{P}$ therefore leaves the reduced
task not invariant but rolled about its own first axis,
\begin{equation}
  \hat{x} \;\longmapsto\; R_{\hat{u}}(\theta)\,\hat{x},
  \qquad \theta = \theta(T, \mathcal{P}),
  \label{eq:residual}
\end{equation}
so what Eq.~\eqref{eq:planner-equivariance} still asks of a planner in this frame
is equivariance to $\mathrm{SO}(2)$, not to nothing.
...
\subsection{Why the remainder is not removable}
