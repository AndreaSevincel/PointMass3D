...
predict what the symmetry bought it. We also calibrate how large the frame's
...
\begin{itemize}\itemsep2pt
  ...
  other.} Neither of the two run to convergence recovers a point, and the third
was already not significant at a fixed budget. The constrained architecture
...
$d_{\mathcal{O}} : \mathbb{R}^3 \to \mathbb{R}$. For $\mathcal{C} = \SE$ we
overload $d_{\mathcal{O}}(p)$ to mean the minimum, over the spheres composing the
body at pose $p$, of the field at each centre less that sphere's radius, so
$\rho$ is absorbed into the body and the constraint below reads unchanged. A trajectory is a waypoint
...
\begin{equation}
  ...
  d_{\mathcal{O}}(p) \ge \rho \;\; \forall\, p \in \gamma(x) \,\big\},
  \label{eq:feasible}
\end{equation}
...
not plateaued. Seed-matched, the difference grows
...
fixed budget of $20$ epochs.} The world-frame model moves $2.5$ points across a
$12.5\times$ increase in environments, from $12.9$ to $15.4$, while the
reduction runs from $19.2$ to $45.5$ without plateauing. Holding the budget fixed is what makes representation the only
...
reduction, so neither had stopped improving when the runs ended. The density rows are freshly generated
