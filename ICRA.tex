...
\begin{abstract}
\noindent
Equivariance to rigid-body transformations is a cornerstone of generalisation in
...
planning query names a start and a goal, and those two points determine a frame
...
worth under a point each at convergence, though the constrained architecture
reaches any given level two to three times sooner. Two models that are equally
equivariant differ by $28$ points, so how equivariant a planner became does not
predict what the symmetry bought it. We also bound the result from above. Giving
each waypoint its distance and direction to the nearest obstacle is worth $+40.0$
points where the frame is worth $+36.5$, and the two are not substitutes.
\end{abstract}
...
by one of the three mechanisms above, and we build all three. Fig.~\ref{fig:teaser} takes the group apart
one stage at a time.
...
anything. Frames read off the start and the goal are common practice in
...
$d_{\mathcal{O}} : \mathbb{R}^3 \to \mathbb{R}$. A trajectory is a waypoint
sequence $x = [p_1, \dots, p_N] \in \mathcal{C}^N$, and the feasible set of
$\mathcal{P}$ is
\begin{equation}
  \mathcal{S}(\mathcal{P}) =
...
\end{equation}
The denominator never vanishes: $k$ forces $\hat{u}_k^2 \le \tfrac{1}{3}$, so
$\|\hat{u} \times e_k\|^2 = 1 - \hat{u}_k^2 \ge \tfrac{2}{3}$. Let
$R_{\mathcal{P}} \in \mathrm{SO}(3)$ be the rotation with
...
analytic signed distance field, and every segment between consecutive waypoints
is resampled at a spacing of $0.01$ before it is tested, so the check is
continuous to a third of the robot radius rather than evaluated at the $64$
waypoints alone. A trajectory
...
gains $2.5$ points, from $12.9$ to $15.4$. The reduction gains $26.2$, from
$19.2$ to $45.5$, and has not plateaued. Seed-matched, the difference grows
monotonically at every size: $+6.37 \pm 0.19$, $+20.87 \pm 1.56$,
$+25.85 \pm 0.37$ and $+30.08 \pm 0.38$.
...
\caption{\textbf{Held-out collision-free rate against training-set size, at a
fixed budget of $20$ epochs.} The world-frame model gains $2.5$ points across a
$12.5\times$ increase in environments and the reduction gains $26.2$ without
plateauing. Holding the budget fixed is what makes representation the only
variable, and it is also why these are not convergence levels: trained out, the
$250$-environment reduced model reaches $55.8$, above the $51.10$ quoted at $60$
environments elsewhere. Every world-frame and reduction cell is a mean over three
seeds.}
\label{fig:scaling}
\end{figure}
...
\caption{\textbf{Held-out collision-free rate against training epochs, $60$
environments.} The constrained backbone reaches any given level two to three
times sooner than the unconstrained model in the same frame, and the two converge
to within $0.15$ points. Seed-matched over three seeds.}
\label{fig:conv}
\end{figure}
...
$51.27$ against $51.12$ on the same three seeds, so the $+0.15$ of
Table~\ref{tab:mechanisms} is that difference and not $51.27$ minus the
three-seed mean $51.10$ above it.}
\label{tab:twobytwo}
\end{table}
...
epochs and only the reduced one clears it. The second budget is where
best-validation loss sits for both models, at epochs $155$ and $159$, so neither
had stopped improving when the runs ended. The density rows are freshly generated
