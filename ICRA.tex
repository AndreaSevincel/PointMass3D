...
\newcommand{\lipA}{The task of motion planning is equivariant to rigid-body transformations, but the field the flow matching network learns is not. There are currently three techniques to restore equivariance: in the data, in the operator, or in the weights.}

\newcommand{\lipB}{We ask what the task gives us before any of them. The task itself already supplies us with a frame in the workspace, by placing the origin at the midpoint of the start and goal and aligning the first axis with the direction between the two points, which removes five of the six degrees of freedom of SE(3) exactly, at initialisation and at no cost. On 3D workspaces holding 40 obstacles, this raises the held-out collision-free rate from 14.6\% to 51.1\%. A straight line scores 15.6\% on the identical problems, and the world-frame model does not beat it. A control that shows the obstacle encoder the start and goal without changing frame also reaches 15.6\%, so it is the frame and not the conditioning that moves the model off the floor. One degree of freedom survives, a roll about that axis, and no continuous rule can fix it. Augmentation, frame averaging and an SO(2)-equivariant architecture are worth +0.8, +0.68 and +0.15 points, and the constrained backbone reaches any level two to three times sooner, so the symmetry buys optimisation rather than accuracy. The frame is also not the largest effect available: a per-waypoint signed distance and gradient is worth +40.0 against the frame's +36.5, and the frame still adds +24.8 alongside it.}
\newcounter{fillc}
\newcommand{\filler}[1]{%
  \setcounter{fillc}{0}%
...
% EXAMPLE ONLY. Five paragraphs on the plan that was here before: the question,
% what the frame removes, the three mechanisms, calibration, the hierarchy.
% Rewrite in your own voice; the structure is the part worth keeping.
...
they are posed. Sampling-based and
optimization-based algorithms are
...
the same manoeuvre at every position and orientation, and restoring equivariance
in these models remains an open problem.

Current approaches place the symmetry in one of three parts of the pipeline. In
the data, through augmentation. In the inference operator, through frame
averaging or canonicalization. Or in the weights,
through architectures equivariant by
construction, as
adopted in recent robot learning
systems. All
three modify the model. None asks how much of the symmetry the problem statement
already supplies before any model is built. We build all three mechanisms on one
benchmark, hold architecture, data and training budget fixed, and measure each
against what the problem supplies on its own.
...
rather than leaving it fixed. What survives is exactly that roll. $\SE$ collapses
to $\mathrm{SO}(2)$, not to nothing. Fig.~\ref{fig:teaser} takes the group apart
one stage at a time.

No continuous rule can fix the remaining roll, so it has to be handled by one of
the three mechanisms above. Roll augmentation is worth $+0.8$ points, frame averaging $+0.68$, and
an exactly $\mathrm{SO}(2)$-equivariant backbone $+0.15$, all seed-matched at
convergence. Three unrelated ways of imposing the same symmetry recovering under
a point each is a stronger statement than any one of them makes alone. The
constrained architecture is not idle. It reaches any given level two to three
times sooner than the unconstrained one, and it carries $19.5\%$ more parameters,
so that comparison is not confounded by capacity. The symmetry buys optimisation
and not accuracy.

A collision-free rate on a new benchmark means nothing without a scale, so we
measure both ends on the identical $500$ held-out problems. A straight line from
start to goal is collision-free on $15.6\%$ of them. The world-frame model,
trained to a plateau, reaches $14.60\%$. Classical planners reach $74\%$ to
$100\%$ in under $0.4$\,s on one CPU core. We also bound our own contribution
from above. Supplying each waypoint with the signed distance to the scene and its
gradient, a featurisation of data the network already receives, is worth $+40.0$
points where the frame is worth $+36.5$. The representation change studied here
is not the largest effect available on this benchmark. It is not a substitute for
that information either: with local geometry present the frame still adds
$+24.8$.

Three levels appear in this paper and they are not interchangeable. The headline
is $51.10\pm0.25$ against $14.60\pm0.20$: sixty environments, trained to a
plateau, three seeds, and the setting in which every mechanism comparison below
is made. The $45.5$ of the scaling grid runs at a fixed $20$-epoch budget and
answers how the gap moves with data rather than how well the method does. The
$55.8$ at $250$ environments is the largest number we measured and rests on a
single seed.
