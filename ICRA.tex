...
$d_{\mathcal{O}} : \mathbb{R}^d \to \mathbb{R}$. The objective is to learn a
generative model $p_\theta$ that produces a collision-free waypoint sequence
$x = [p_1, \dots, p_N] \in \mathbb{R}^{N \times d}$ conditioned on $\mathcal{P}$,
with $p_1 = \svec$, $p_N = \gvec$, and $d_{\mathcal{O}}(p_i) \ge \rho$ for every
$i$, where $\rho$ is the robot radius.

The task carries an exact symmetry. For any rigid transformation $T \in \SE$, the
transformed task $T\mathcal{P} = (T\svec, T\gvec, T\mathcal{O})$ has as its
solutions exactly the transformed solutions of $\mathcal{P}$:
\begin{equation}
  x \text{ solves } \mathcal{P}
  \;\Longleftrightarrow\;
  Tx \text{ solves } T\mathcal{P},
  \label{eq:equivariance}
\end{equation}
so a generative planner should satisfy
$p_\theta(Tx \mid T\mathcal{P}) = p_\theta(x \mid \mathcal{P})$. We use
conditional flow matching~\cite{lipman2023fm}, which learns a time-dependent
velocity field $v_\theta(x_t, t, \mathcal{P})$ whose integral transports a simple
prior to $p_\theta(x \mid \mathcal{P})$. Nothing in that objective enforces
Eq.~\eqref{eq:equivariance}.

\subsection{Decomposing the group}
\label{sec:decompose}
We formulate motion planning in a workspace $\mathbb{R}^d$ ($d=3$ throughout, $d=6$ for the rigid-body domain of Sec. V). A task is defined by the tuple $\mathcal{P} = (\mathbf{s}, \mathbf{g}, \mathcal{O})$, where $\mathbf{s}, \mathbf{g} \in \mathbb{R}^d$ are the start and goal configurations and $\mathcal{O}$ is a set of convex obstacles inducing a signed distance field $d_{\mathcal{O}} : \mathbb{R}^d \to \mathbb{R}$. The objective is to learn a generative model $p_\theta$ that produces a collision-free waypoint sequence $x = [p_1, \dots, p_N] \in \mathbb{R}^{N \times d}$ conditioned on $\mathcal{P}$, with $p_1 = \mathbf{s}$, $p_N = \mathbf{g}$, and $d_{\mathcal{O}}(p_i) \ge \rho$ for every $i$, where $\rho$ is the robot radius.
