...
\section{Method}
In this section, we begin by formally defining the generative motion planning objective and the symmetry it carries (Sec. 3.1). We then decompose SE(3) into the part a planning query determines and the part it does not, and state precisely what survives that reduction (Sec. 3.2). To establish that the remainder cannot be removed by the same route, we give two independent obstructions, one topological and one distributional (Sec. 3.3). Finally, we enumerate the three places a symmetry can be imposed, in the training data, in the inference operator, and in the weights (Sec. 3.4), and define the non-equivariance residual that bounds what the second of them can remove.
