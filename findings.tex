...
statement of what the method achieves.}
...
\finding{D3}{\HEAD} At convergence the world-frame arm is \emph{below} the
straight-line baseline.
\ev{world frame 14.6 vs straight line 15.6, per sample}
\nt{Sharper than the earlier ``indistinguishable from joining the endpoints''.
Do not soften it back --- see \texttt{L7}.}
...
40  & 44.6 & 37.9 & $+6.7$ \\
100 & 48.7 & 45.8 & $+2.9$ \\
300 & 51.27 & 51.10 & $+0.17$ \\
\bottomrule
\end{tabular}
\end{center}
\ev{final row seed-matched over THREE seeds: 50.9/51.6/51.3 against
50.8/51.6/50.9, i.e. +0.1, 0.0, +0.4. Reaches at epoch 10 what the unconstrained
arm needs epoch 40 for, and at epoch 40 what it needs epoch 90 for.}
\nt{The +0.05 quoted before the third seed landed was two seeds. The conclusion
is unchanged and the number is not: quote \emph{+0.17}.}
...
\end{tabular}
\end{center}
\ev{250-env cells are n=1 and matched at 80 epochs across arms}
\nt{The reduction is flat across a 4x increase in environments at matched
budget; local geometry gains 9 points. Its lead grows from 3.4 to 11.8. Sharpest
form: the world-frame arm with local geometry reaches \emph{57.3 at epoch 20},
beating the canonicalised arm without it at epoch 300 (55.8). The mechanism is
consistent with [[E2]]: the global encoder pools 40 obstacles into one vector
applied identically to every waypoint, so more environments means more scenes
through the same bottleneck, while local geometry bypasses it.}
...
\finding{N1}{\OPEN} The four local-geometry cells are single-seed.
\ev{everything else carries three seeds; two more rounds of about 4 h fixes the
two cells that carry the argument}

\finding{N2}{\OPEN} The equivariant arm has two seeds, not three.
