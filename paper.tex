...
straight line, measured on the identical problems. Per sample, the default
configuration a practitioner would reach for does not beat a straight line. With
...
velocity field lies outside the equivariant subspace, and that fraction
\emph{decreases} with scale. Because frame averaging is the orthogonal projection
...
We report classical planners on the same $500$ problems to fix the scale
(RRT-Connect $99.6\%$, CHOMP $88.8\%$, all under $0.4$\,s on one CPU core). The
learned sampler does not approach them, and we present this as a controlled study
of representation choices, not as a planner proposal.
\end{abstract}
...
for it --- the model's own untrained prior, sampled $N$ times.
...
\begin{tabular}{lrrrr}
\toprule
Planner & Free (\%) & Clear. & Len. & s/query \\
\midrule
straight line               & 15.6 & 0.055 & 1.68 & $<0.001$ \\
TrajOpt                     & 74.0 & 0.026 & 1.82 & 0.352 \\
CHOMP                       & 88.8 & 0.019 & 1.83 & 0.394 \\
RRT-Connect                 & 99.6 & 0.017 & 1.97 & 0.284 \\
expert (RRT+CHOMP)          & 100.0 & 0.037 & 1.95 & 0.342 \\
\midrule
...
samples per query, so the columns are not a like-for-like speed comparison.
Clearance and length are averaged over solved problems only. The learned rows are
...
Whatever the world-frame arm has learned, on a per-sample basis it is worth
nothing over connecting the endpoints with a line. This is the sharpest available
