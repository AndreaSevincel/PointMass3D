...
three seeds, giving seed-matched differences of $+6.37\pm0.19$ and
$+20.87\pm1.56$; the $150$- and $250$-environment rows are single seed. The last
column is the deployment metric, best-of-$20$ behind a collision check}
\label{tab:main}
\end{table}
...
\textbf{Two points, not a plateau --- but the second domain goes further.} In
this domain neither arm has been trained to a plateau, so the strongest form of
the objection --- that the gap reflects convergence \emph{rate} rather than
representation --- is answered here by two points rather than by a limit. What
the two points establish is the direction: the gap \emph{widens} from $20.8$ to
$28.0$ points under $3\times$ the budget, so if undertraining were manufacturing
the gap it is the flat arm that would have the headroom, and it does not.

The rigid-body domain of Sec.~\ref{sec:se3} tests the same thing over an
$8\times$ budget range and reaches the same conclusion, which is why we do not
rest this claim on the $3\times$ comparison alone. A matched convergence study in
the point-mass domain is running at the time of writing;
Sec.~\ref{sec:limits} states what each outcome would mean for the framing.
