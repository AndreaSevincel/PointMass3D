...
$+0.68$, and an exactly $\mathrm{SO}(2)$-equivariant backbone $+0.05$. Three
...
idle: it reaches in a quarter of the epochs what the unconstrained one needs to
converge, so the symmetry buys optimisation rather than accuracy.
...
frame, and reaches it in a quarter of the epochs; it does not show that
...
$\mathrm{SO}(2)$-equivariant backbone $+0.05$, all seed-matched at convergence.
...
fourfold saving in optimisation, reaching by epoch $10$ what the unconstrained
...
not, predict the fourfold optimisation saving in (iii).
...
reaches at epoch $10$ what the unconstrained one needs epoch $40$ to reach, and
at epoch $40$ what it needs epoch $90$ to reach --- roughly a fourfold saving in
optimisation. By convergence the advantage is $+0.05$ points seed-matched, which
...
is worth $+0.8$, frame averaging $+0.68$, and an equivariant architecture $+0.05$
...
equivariant backbone is worth $+0.05$ points without local geometry and $+2.8$
...
backbone is worth $+0.05$ points at convergence, so the conclusion the bound
would have suggested happens to hold --- but it is worth roughly a fourfold
...
the main grid now carry three seeds; several ablations, and the converged DDPM
cells, remain single seed and are reported as point estimates.
...
\emph{(2) Additional seeds on the local-geometry arms}, which are single-seed
where every other comparison here carries three. \emph{(3) Convergence at $250$ environments}, so the
