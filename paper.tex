...
find. Comparing at a matched $80$-epoch budget so that scale is the only variable,
the reduction is worth $+22.3$ points alongside local geometry at $60$
environments and $+17.1$ at $250$ (single seed at the larger scale). Local
geometry is what scales here --- it gains $+9$ points across that range while the
reduction alongside it loses $5$ --- and the reduced arm with local geometry is
close to saturating at roughly $80\%$, so the world-frame arm has room to close
on it that the reduced arm no longer has. The reduction remains worth a great
deal at both scales and costs nothing; but a reader extrapolating to much larger
datasets should expect this particular margin to narrow, not hold.
...
the main grid now carry three seeds. The local-geometry arms of
Table~\ref{tab:localgeom} carry three seeds for the reduction and two for the
world-frame and equivariant arms; every $250$-environment cell, several
ablations, and the converged DDPM cells remain single seed and are reported as
point estimates.
