...
\textbf{The one comparison we cannot make.} Corollary~\ref{cor:budget} bounds
post-hoc symmetrisation of a given field; a constrained architecture searches a
different hypothesis class and is not covered by it. We are careful not to borrow
the bound for that purpose, and Sec.~\ref{sec:twosigns} is why: $r$ does not
forecast what a symmetry mechanism is worth, so its smallness cannot be turned
around into a prediction that an $\mathrm{SO}(2)$-equivariant backbone would gain
little. That experiment is the largest single gap in this work and we state it as
untested rather than as unpromising.
