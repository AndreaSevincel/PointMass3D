...
mechanisms tested, canonicalisation accounts for the effect: the next largest is
an order of magnitude smaller, and the two built for the sixth degree of freedom
are smaller still. The $\Delta$ column is against the arm without that mechanism,
so the rows are not additive}
\label{tab:ablation}
\end{table}
