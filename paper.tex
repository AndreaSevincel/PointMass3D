...
\label{prop:moments}
Let obstacle centres be i.i.d.\ uniform on a cube centred at the origin, let
$\varphi_j$ be their angles in the plane normal to $\xhat$, and let the weights
$w_j$ depend only on radius. Then the angular moments
$c_m=\sum_j w_j e^{im\varphi_j}$ satisfy
\begin{equation}
  \mathbb{E}[c_m] = 0 \quad\text{for every odd } m
  \text{ and every } \xhat \in S^2 .
  \label{eq:moments}
\end{equation}
In particular $\mathbb{E}[c_1]=0$: the first-moment gauge is degenerate in
expectation, for every start--goal direction.
\end{proposition}

\begin{proof}
The cube-uniform law is invariant under the central inversion $\mathbf{p}\mapsto-\mathbf{p}$,
which fixes every radius and hence every $w_j$, and which maps
$\varphi_j\mapsto\varphi_j+\pi$. So $e^{im\varphi_j}\mapsto(-1)^m e^{im\varphi_j}$,
giving $\mathbb{E}[c_m]=-\mathbb{E}[c_m]$ for odd $m$.
\end{proof}
...
This is enough for the purpose. The scene-derived gauge one would actually build
--- ``point $\yhat$ at the obstacle mass'' --- is the $m=1$ moment, and it is
degenerate in expectation for every $\xhat$. Under the null,
$|c_1|^2/\sum_j w_j^2$ is approximately $\mathrm{Exp}(1)$ with median
$\approx0.69$, so near-degenerate configurations are the typical case rather
than a rare tail. A gauge built from an even moment escapes
Eq.~\eqref{eq:moments}, but not Proposition~\ref{prop:hairy}: an $m=2$ moment
defines a \emph{director} rather than a vector, and a continuous director field
on $S^2$ is obstructed for the same Euler-characteristic reason as a vector
field. Both routes to a canonical roll are closed, for unrelated reasons.
...
instead raises the held-out collision-free rate from $31.3\%$ to $\mathbf{42.4\%}$,
with minimum clearance improving from $-0.031$ to $-0.013$ and endpoint error from
$0.0024$ to $0.0022$. Three times the optimisation, at identical data and
identical capacity, is worth $+11.1$ points --- an order of magnitude more than
either mechanism built for the sixth degree of freedom.
...
Second, it qualifies the reading of Fig.~\ref{fig:scaling}. The reduced arm at
$60$ environments and $60$ epochs ($42.4\%$) is close to the same arm at $250$
environments and $20$ epochs ($45.6\%$), so on this benchmark optimisation and
environment count are substantially substitutable, and environments are
\emph{not} straightforwardly the binding constraint. What the scaling figure
measures is how each arm converts data into performance \emph{at a fixed and
insufficient budget}. We think the comparison remains sound --- both arms get the
same budget, and the world-frame arm is flat under a $12.5\times$ increase in
data, which is not the signature of a model that merely needs longer --- but the
epoch-matched control at $60$ epochs is the experiment that would settle it, and
it has not been run.
...
capacity, three times the optimisation is worth $+11.1$ points of held-out
