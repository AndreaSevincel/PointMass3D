...
\begin{abstract}
\noindent
Motion planning is exactly equivariant under rigid transformations, and a large
literature builds that symmetry into network architectures. We ask where the
symmetry is best placed, and answer by decomposition rather than by advocacy:
we enumerate the mechanisms available, build all of them on one benchmark with
architecture, data and budget held fixed, and measure them against each other.

A planning query supplies its own frame. Placing the origin at the midpoint of
$\svec$ and $\gvec$ and aligning the first axis with the start--goal direction
removes five of the six degrees of freedom of $\SE$ exactly, at initialisation,
at no computational cost. On a cluttered 3D point-mass benchmark, trained to
convergence, this raises the held-out per-sample collision-free rate from
$14.60\pm0.20\%$ to $51.10\pm0.25\%$ across three seeds. The world-frame model
does not beat the $15.6\%$ obtained by joining start to goal with a straight line
on the identical problems. A paired bootstrap over environments puts the
difference at $-1.02$ points, $95\%$ CI $[-3.06, +0.90]$, so training on world
coordinates buys nothing over connecting the endpoints, and we do not claim it
loses to them.

The sixth degree of freedom, a roll about that axis, admits no continuous
canonical rule; we give two independent obstructions, one topological and one
distributional. It can therefore be handled only by symmetry, and there are
exactly three places to put a symmetry: in the data, in the inference operator,
or in the weights. Roll augmentation is worth $+0.8$ points, frame averaging
$+0.68$, and an exactly $\mathrm{SO}(2)$-equivariant backbone $+0.15$. Three
unrelated mechanisms recovering almost nothing is a considerably stronger
statement than any one of them makes alone. The constrained architecture is not
idle: it reaches any given level two to three times sooner than the
unconstrained one, so the symmetry buys optimisation rather than accuracy.

We explain that null result rather than merely reporting it. The
non-equivariance residual, the disagreement among rotated copies of the learned
field, is $1.7\%$; frame averaging is exactly the orthogonal projection onto the
equivariant subspace, so $1.7\%$ bounds what any post-hoc symmetrisation of this
model can remove. We also measure what the residual does \emph{not} do: two models
that are equally equivariant differ by $28$ points of held-out performance, so it
does not forecast what a symmetry mechanism is worth.

Finally we bound our own contribution from above. Supplying each waypoint with
the signed distance to the scene and its gradient, a featurisation of data the
network already receives rather than privileged information, is worth $+40.0$
points against the world frame where the reduction is worth $+36.5$. The
representation change studied here is not the largest effect available on this
benchmark. It is also not a substitute for local geometry: with that information
present, the reduction is still worth $+24.8$.
\end{abstract}
...
benefit is available without it, and we answer it by building the constrained
route as well and comparing the two directly. We are deliberate about what that
comparison does and does not license. It shows that on this problem a
constrained backbone reaches the same ceiling as an unconstrained one given a
frame, and reaches it two to three times sooner; it does not show that
constrained layers cost expressiveness or generalise worse, and we make no such
claim.
...
alternative carries no signal.} No
...
\textbf{(iii) All three mechanisms for the sixth, measured against each other.}
A symmetry can be imposed in exactly three places: in the data, in the inference
operator, or in the weights. We build all three on the same benchmark. Roll
augmentation is worth $+0.8$ points, frame averaging $+0.68$, and an exactly
$\mathrm{SO}(2)$-equivariant backbone $+0.15$, all seed-matched at convergence.
The backbone is verified equivariant to $1.2\times10^{-6}$ on an \emph{untrained}
network and carries $19.5\%$ \emph{more} parameters than the unconstrained one,
so the comparison is not confounded by capacity, and a negative control confirms
it is not equivariant off-axis, so the verification cannot pass vacuously. The
constrained architecture does buy something the other two do not: roughly a
two- to threefold saving in optimisation, reaching by epoch $10$ what the unconstrained
model reaches by epoch $40$. Three unrelated mechanisms agreeing that the residual
roll is nearly exhausted is a considerably stronger statement than any one of
them makes alone.

\textbf{(iv) An explanation for the null result, and a limit on it.} Rather than
infer inertness from flat curves --- a weak conclusion --- we measure the
quantity frame averaging exists to remove. The \emph{non-equivariance residual},
the disagreement among rotated copies of the velocity field, is $1.7\%$ and is
flat across a $12.5\times$ range of training data. Since frame averaging is the
orthogonal projection onto the equivariant subspace (Proposition~\ref{prop:fa}),
the error it can remove is bounded by that $1.7\%$ (Corollary~\ref{cor:budget}),
while the model is failing on most of its samples: the model's error is
overwhelmingly equivariant error. We are equally explicit about what the residual
does not do. It does not forecast what a mechanism is worth --- two models with
indistinguishable residuals differ by $28$ points --- and it did not, and could
not, predict the optimisation saving in (iii).

\textbf{(v) Calibration against floors and ceilings, including an upper bound on
our own contribution.} A collision-free rate is
...
$15.6\%$ of problems, which the world-frame model does not beat. The
classical planners ($74$--$100\%$, all under $0.4$\,s on a single CPU core) bound
the other side and show that the learned sampler, at $51.1\%$, is not competitive
...
world-frame model is at the straight-line floor per sample, but well above the
untrained prior per query, so it has learned sample \emph{diversity} without
learning sample \emph{quality}.

\textbf{Which number is the headline.} Three appear in this paper and they are
not interchangeable, so we fix the hierarchy here. The headline is
$\mathbf{51.10\pm0.25}$ against $14.60\pm0.20$: sixty environments, trained to a
plateau, three seeds, and the setting in which every mechanism comparison in
Sec.~\ref{sec:arch} and Sec.~\ref{sec:localgeom} is made. Two others are
subordinate to it and are labelled as such wherever they appear.
$45.5\%$ is the $2\times4$ scaling grid of Sec.~\ref{sec:scaling}, run at a
fixed $20$-epoch budget so that four data scales are compared at equal
optimisation; it answers how the gap moves with data, not how well the method
does. $55.8\%$ is the best number we measured, at $250$ environments trained to
convergence, and it is a single seed, which is why it is not the headline
despite being the largest.

A reader may reasonably ask why a generative planner is worth studying on a
problem with an exact SDF, where RRT-Connect solves essentially everything in a
third of a second. We answer it here rather than by implication. PointMass3D is
an analytical sandbox chosen \emph{because} the SDF is exact: an analytic
collision checker makes the floor and the ceiling computable on the identical
problems, so a representation change can be measured against both rather than
only against other configurations of itself. On this benchmark one should use
RRT-Connect, and we say so in Sec.~\ref{sec:baselines}. The measurement is worth
making anyway because the quantity being isolated --- what a generative planner
gains from being shown its query in a canonical frame --- is a property of the
representation rather than of the collision checker, and it transfers to the
settings where generative planners are the right instrument: learned or
perceptual cost fields, contact-rich dynamics, multi-modal solution sets. None of
those admit an analytic SDF, and a result of this kind can only be isolated where
the confounds are removable.

We regard (iii) as the transferable contribution, and we are deliberately narrow
about what the residual adds to it. Two things survive scrutiny. One is
Corollary~\ref{cor:budget}, which follows from the projection property already
established in~\cite{elesedy2021strict,puny2022fa} and bounds exactly what
post-hoc symmetrisation of a given field can remove. The other is a measured
negative: across three models of this benchmark $r$ does \emph{not} predict what a
symmetry mechanism is worth, and two models with indistinguishable residuals
differ by $28$ points (Table~\ref{tab:twosigns}), because one of them fixed the
representation and the other only learned the symmetry. We do not claim more.
An earlier draft recommended $r$ as a general pre-flight diagnostic; the cone
sweep that would have established it as predictive failed and is confounded
(Sec.~\ref{sec:inert}), so the honest position is a bound plus a counterexample to
the obvious decision rule, not a tool we are advertising.
...
Sec.~\ref{sec:twosigns}, where two models with indistinguishable residuals differ
by $28$ points of held-out performance.

This is also a consistency check on Proposition~\ref{prop:exact} for a
\emph{trained} model. For the reduced model the $\SE$ residual is $0.0186$ at
...
this model ($0.0174$) differs slightly from the $K{=}9$ and $K{=}32$ values here;
...
architecture would gain little: Sec.~\ref{sec:twosigns} exhibits two models that
...
with $8$ Euler steps. The two models are:
...
Both models use the twelve-dimensional oriented-box representation, so it is not a
...
difference of two models. We checked that assumption rather than asserting it: a
...
\textbf{Loss is not comparable across models.} The treatment regresses targets in
...
rows are the converged $60$-environment models of Sec.~\ref{sec:budget}, three-seed
means, which is the headline setting of this paper; the $2\times4$ scaling grid
of Sec.~\ref{sec:scaling} runs at a fixed $20$-epoch budget and reaches $45.5$,
and a single-seed run at $250$ environments trained to convergence reaches
$55.8$. Classical
...
\caption{Where the learned planner actually sits. The world-frame model lies on the
straight-line floor; the reduction lifts it by $36.5$ points; the classical
...
direct path is usually blocked. The world-frame model converges to $14.60\%$ per
sample. A paired bootstrap over environments puts the difference at $-1.02$
points with a $95\%$ interval of $[-3.06, +0.90]$, so the two are not separable.
On a per-sample basis, whatever the world-frame model has learned is worth nothing
...
problem. We ran that model at all four scales (Fig.~\ref{fig:scaling},
...
world-frame model within $2$ points of the straight-line floor. Making equivariance
...
carefully. Per sample, that model is at the straight-line floor. Per query at $N=20$
...
initialisation solves $88.8\%$. The learned sampler at $51.1\%$ is not competitive
...
change is worth $36.5$ points while all three mechanisms built for the residual
symmetry are worth under a point each, none of them distinguishable from zero at
three seeds. That contribution does not require the planner to win, and we do not
claim it does.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig_cascade.pdf}
\caption{$\SE$ taken apart one stage at a time, at $250$ training environments.
Every bar is a measured model rather than a term in a decomposition: the second is
a translation-only reduction, trained and evaluated like the others. The
increments between them are what each group of degrees of freedom is worth. The
bottom bar adds all three mechanisms built for the residual $\mathrm{SO}(2)$ ---
roll augmentation, frame averaging and an equivariant backbone --- and is
deliberately drawn on the same scale: its near-coincidence with the bar above is
the paper's second finding. The dashed line is a straight segment from start to
goal on the identical problems, which the world-frame model does not beat.}
\label{fig:cascade}
\end{figure*}
...
model with one component removed}
\label{fig:scaling}
\end{figure}
...
$\mathbf{+30.08\pm0.38}$ points, measured across three seeds per model.

The uncertainty is quoted from the seed spread in the same cell, which is what
makes it a test rather than an extrapolation. Both models were run at seeds
...
\begin{tabular}{@{}l r r >{\raggedright\arraybackslash}p{1.75cm}@{}}
\toprule
Mechanism & $20$ ep. & conv. & Assessment \\
\midrule
...
Equivariant backbone              & ---     & $+0.15$ & $n{=}3$; $2$--$3\times$ faster \\
\bottomrule
\end{tabular}
\caption{Decomposition by mechanism on the same held-out set, in percentage
points against the model without that mechanism --- so the rows are not additive,
and the two indented rows decompose the row above them. Canonicalisation accounts
for the effect at either budget, and both of its parts matter: recentring and
axis alignment contribute $41\%$ and $59\%$. The two columns are the same
mechanisms measured at $20$ epochs and at convergence; reading only the first
would put frame averaging at zero, which Sec.~\ref{sec:inert} shows is an
artefact of the budget rather than a property of the mechanism. Dashes are
comparisons we did not run to convergence. The $\SE$-augmentation and
translation-only models are single seed and are therefore differenced against the
SAME seed, never against a three-seed mean}
\label{tab:ablation}
\end{table}
...
is $\pm2$ across-seed standard errors, measured on the treatment model at $60$
...
against $33.5\%$ at $60$ environments. Both models were run at the same three
...
across-model spread swamps it entirely ($0.4$ SE). Frame averaging on the best
...
Rather than infer that from flat curves, we measure $r$. On the best model
...
most $r^2$ of the field's squared magnitude, i.e.\ under $3\times10^{-4}$, while
the model is failing on more than half of its samples. At \emph{this} budget the
remaining error is overwhelmingly equivariant error, which no symmetrisation
reaches.

\textbf{But the budget is doing the work, not the mechanism.} Scoring every
snapshot of the convergence runs at both $K{=}1$ and $K{=}3$ costs nothing extra
and turns that single number into a curve. Across three seeds at $60$
environments the gain rises from $+0.19$ early to $+0.68\pm0.13$ over the final
eight snapshots, positive in every seed, while $r$ sits at $0.0180$--$0.0183$
throughout. At $250$ environments it is steeper still: $+0.22$ at epoch $10$,
peaking at $+1.57$ near epoch $180$, settling around $+0.9$, with $r$ moving only
from $0.0163$ to $0.0173$. The mechanism the $20$-epoch grid cannot distinguish
from zero is worth roughly seven times that on a converged model.
...
error lying outside the equivariant subspace --- the only part symmetrisation can
reach --- grows. The honest claim is therefore not that frame averaging is inert,
but that its value scales inversely with how much equivariant error is left. At
our main grid's budget there is far too much of it for the $r^2$ ceiling to
matter; on a better model it begins to. The ordering of Table~\ref{tab:ablation}
is unchanged either way --- $+0.68$ against the reduction's $+36.5$ --- but
...
$1.56$ points, which governs comparisons between separately trained models, is
simply the wrong noise model for it.
...
quality varies by $17$ points and the benefit grows sevenfold. To vary $r$
instead, we restricted training to a cone of start--goal directions about the
first axis, starving the reduced-frame data of the implicit roll diversity that
makes the field near-equivariant in the first place. The manipulation works: at
cone half-angles of $90$, $45$, $30$ and $15$ degrees, with training-set size and
gradient-step count held fixed, $r$ rises monotonically through $0.0182$,
$0.0192$, $0.0202$, $0.0238$. The frame-averaging benefit over the same sweep is
$+0.02$, $-0.05$, $+0.44$, $-0.25$: scatter about zero, with the largest $r$
giving the most negative gain.

We report that sweep as an attempted decoupling that \emph{failed}, because it
is confounded and saying so is more useful than the conclusion it appears to
support. Narrowing the cone also costs generalisation --- $K{=}1$ falls from
$31.4$ to $20.6$ --- so $r$ and quality move in opposite directions by
construction, and a flat result is consistent both with $r$ being irrelevant and
with $r$ mattering while quality cancels it. Re-scoring every model on a common
$15$-degree cone of held-out queries, a subset of all four training cones so that
no model extrapolates, lifts them all by about $15$ points and confirms that much
of the collapse was distribution shift --- but it does not equalise them: the
spread stays at $12$ points, because an model trained on a narrow cone generalises
worse even \emph{within} that cone despite seeing the same number of
trajectories. And among the three models whose quality does match ($45.0$, $46.0$,
...
cone dial cannot separate them. Read \emph{with} the convergence curve, where $r$
is fixed and quality alone varies, the variable that tracks the benefit is
quality.
...
world-frame model as well: $K$ random rigid motions are applied to the whole
...
\begin{tabular}{@{}l r r@{}}
\toprule
Model ($250$ envs) & $r$ under $\SE$ & Free (\%) \\
\midrule
...
\subsection{The third mechanism: a constrained architecture}
\label{sec:arch}

Sec.~\ref{sec:sixth} distinguishes three places a symmetry can be imposed:
in the data, in the operator, or in the weights. The first two are measured
above. Earlier versions of this work stated the third as untested, and were
careful not to let Corollary~\ref{cor:budget} stand in for it: the bound governs
post-hoc projection of a \emph{given} field, whereas a constrained architecture
searches a different hypothesis class. We now report it.

We built a backbone that is exactly $\mathrm{SO}(2)$-equivariant about the
start--goal axis by construction. Features are separated by irreducible
representation --- the component along the axis is invariant ($m=0$), the pair in
the normal plane rotates ($m=1$) --- and only operations respecting that
separation are permitted: complex-linear channel mixing, obstacle pooling with
attention weights computed from invariants alone, normalisation of the rotating
stream by an invariant, and gating of the rotating stream by scalars. The
property is verified on an \emph{untrained} network, which is the hard case and
the only one that shows equivariance comes from the structure of the weights
rather than from training: the equivariance error is $1.2\times10^{-6}$, and it
survives multiplying every parameter by three. The same test reports
$5.03$ for a rotation off the axis, so a network that ignored its vector inputs
entirely --- which would be trivially equivariant --- fails it.

Capacity was not matched downward. At the shared hyper-parameters the
constrained model carries $2{,}582{,}233$ parameters against the unconstrained
backbone's $2{,}161{,}283$, or $19.5\%$ more. This is deliberate: a constrained
model that is also smaller confounds architecture with capacity, and the
uncharitable reading is the one a reader reaches for. The comparison below is
therefore against a \emph{larger} constrained model, which strengthens rather
than weakens its conclusion.
...
\bottomrule
\end{tabular}
\caption{An $\mathrm{SO}(2)$-equivariant backbone against the unconstrained one,
$60$ environments, identical data and schedule. The constraint buys training
efficiency and not a higher ceiling. The final row is seed-matched over two
runs; the per-seed pairs are $50.9$ vs $50.8$ and $51.6$ vs $51.6$.}
\label{tab:arch}
\end{table}
...
Table~\ref{tab:arch} and Fig.~\ref{fig:convergence} are unambiguous in both
directions. The constrained model
reaches by epoch $10$ a level the unconstrained model needs about $32$ epochs
for, and by epoch $40$ one it needs about $87$ --- a saving of $2.2$ to $3.2$
times in optimisation, largest early and shrinking as both converge. By convergence the advantage is $+0.15$ points seed-matched, which
is zero. The same pattern holds at $250$ environments, where the constrained model
reaches $54.8$ by epoch $80$ against the unconstrained model's $51.6$.

This completes the taxonomy, and the three mechanisms agree. Roll augmentation
is worth $+0.8$, frame averaging $+0.68$, and an equivariant architecture $+0.15$
at convergence. Three mechanisms that impose the same symmetry by three
unrelated means all recover almost nothing, which is a considerably stronger
statement than any one of them alone. It also sharpens the correct reading of
Sec.~\ref{sec:twosigns}: $r$ did not predict this outcome and could not have,
but the outcome is what the residual's smallness would have suggested --- the
inference is unsound even where the conclusion happens to hold.

\subsection{Local geometry, and what the reduction is worth beside it}
\label{sec:localgeom}

An obstacle encoder that max-pools $40$ obstacles into one scene vector, applied
identically to all $N$ waypoints, gives a waypoint no channel through which to
ask what is near \emph{it} --- while collision avoidance is exactly that
question. Sec.~\ref{sec:mechanism} shows the reduction addresses this indirectly,
by making the scene code query-dependent. It can also be addressed directly: we
append to each waypoint the signed distance to the scene and its gradient there,
which for the constrained backbone splits cleanly by irreducible representation
($d$ and the axial component of $\nabla d$ are invariant, the normal component
rotates). This is a featurisation, not an oracle. The signed distance is a
deterministic function of the obstacle primitives the network already receives;
no privileged information enters. What it does assume is that obstacles are
available as explicit primitives at inference, which holds in this benchmark and
fails for a planner operating on perceptual input.
...
\bottomrule
\end{tabular}
\caption{Local geometry against the query frame, at convergence. Every cell is a
mean over two or three seeds. Across-seed standard errors for the four cells of
the main $2\times2$ are $0.20$, $0.25$, $0.01$ and $0.10$: the local-geometry
models are the least seed-sensitive measurements in this work, with the
world-frame model reproducing to $0.01$ points across its seeds.}
\label{tab:localgeom}
\end{table}

Three things follow from Table~\ref{tab:localgeom}, and we state the
uncomfortable one first.

\textbf{Local geometry is the larger single lever.} Against the world frame it
is worth $+40.0$ points where the reduction is worth $+36.5$. The
representation change this paper is about is not the largest effect available on
its own benchmark. We report this because it bears directly on how the
contribution should be read: the claim that survives is that a query-derived
frame is worth a great deal at zero cost, not that symmetry is the dominant
consideration in designing a generative planner.

\textbf{The reduction is not merely compensating for a weak encoder.} That is
the obvious objection to the paragraph above, and it is testable: if the frame
were only a workaround for a scene code that cannot vary with the query, then
supplying per-waypoint geometry should absorb its benefit. It does not. With
local geometry present the reduction is still worth $+24.8$ points, and the two
world-frame seeds behind that figure agree to $0.05$. We ran this cell
specifically to find out whether it would overturn our main result, and report
that it did not.

Its value does decay with data, which we state rather than leave for a reader to
find. Comparing at a matched $80$-epoch budget so that scale is the only
variable, and differencing within seed, the reduction is worth
$\mathbf{+22.37\pm0.46}$ points alongside local geometry at $60$ environments and
$\mathbf{+17.24\pm0.17}$ at $250$, two seeds at each scale. The $5.1$-point decay
is an order of magnitude larger than either standard error, so it is measured
rather than suggested. Local geometry is what scales here --- it gains $+11$
points across that range while the reduction alongside it loses $5$ --- and the
reduced model with local geometry is close to saturating at roughly $80\%$, so the
world-frame model has room to close on it that the reduced model no longer has. The
reduction remains worth a great deal at both scales and costs nothing; but a
reader extrapolating to much larger datasets should expect this margin to narrow,
not hold.

\textbf{The two are sub-additive, and the constraint finally pays.} Treating the
effects as independent predicts $14.6+36.5+40.0=91.1$; the measured value is
$79.37$. Both address the same underlying difficulty --- relating query geometry
to scene geometry --- and neither subsumes the other. Against this, the
equivariant backbone is worth $+0.15$ points without local geometry and $+2.8$
with it, both seed-matched. That interaction is what the architecture predicts: the rotating stream
has nothing to act on until directional information reaches individual
waypoints, and once it does, enforcing the symmetry on that stream is no longer
free of consequence.

\subsection{How much of the level is training budget}
\label{sec:budget}

Every model above is trained for $20$ epochs, and that budget is not enough. Holding
the data fixed at $60$ environments and training the reduced model for $60$ epochs
...
model at $20$ epochs is substantially underfit, so the $45.5\%$ grid value
...
worth almost nothing to the world-frame model. Running it at $60$ epochs on the
...
world-frame model's flatness is not an artefact of an insufficient budget, because
...
control. The reduction rises from $79.6\%$ to $87.2\%$, while the world-frame model
...
sample. Tripling the optimisation of the world-frame model made it worse on the
...
\textbf{Trained to a plateau, the world-frame model does not beat the straight
line.} The $3\times$ comparison above answers the convergence-rate objection with
a trend rather than a limit, so we ran both models at $60$ environments to $300$
...
\toprule
Epochs & $20$ & $60$ & $150$ & $300$ \\
\midrule
World frame               & $13.2$ & $14.4$ & $14.2$ & $14.60\pm0.20$ \\
$(\svec,\gvec)$ reduction & $31.4$ & $42.3$ & $48.3$ & $\mathbf{51.10\pm0.25}$ \\
\midrule
...
Every cell is a mean over three seeds, and the $\pm$ on the converged column is
the across-seed standard error --- the right noise model here, since both models
are trained, run at the same three seeds, and differenced within seed. Both models
are flat by the end: the control gains $0.1$ points over the final ten epochs and
the reduction $0.2$, so this is a converged comparison rather than a comparison
of convergence rates.
Three things follow.

\textbf{The world-frame model does not beat the straight line.} It converges to
$14.60\pm0.20\%$ against the $15.6\%$ of Sec.~\ref{sec:baselines}, measured on
the identical problems. Quoting the across-seed spread here would overstate the
result: it describes variation between training runs, not the uncertainty of a
comparison against a fixed baseline. The appropriate test is paired, since both
models are evaluated on the same $500$ problems and both track per-environment
clutter, so we resample environments and recompute the difference within each
replicate. That gives $-1.02$ points with a $95\%$ interval of
$[-3.06, +0.90]$ and a standard error of $1.01$ --- less than half the $2.2$ of
either rate resampled alone, and still spanning zero. The defensible claim is
therefore that training on world coordinates does not beat joining the endpoints
with a line, not that it loses to it. At $20$ epochs even that was unavailable
($15.37\pm0.36$): the extra optimisation is what makes the comparison resolvable,
not what causes the deficit, and a reader could previously always answer ``it is
undertrained''.

The gap \emph{widens} monotonically with budget, from $18.2$ to $36.5$ points.
Undertraining was not manufacturing it; if it had been, the flat model would be the
one with headroom.
...
$250$ environments sharpens it. Over $300$ epochs the world-frame model moves from
...
global vector. The comparison row that carries information is the
\emph{full conditioning}, which does contain $(\svec,\gvec)$ and still tops out
at $0.07$ over four orders of magnitude of regularisation. We are deliberate
about what that does and does not establish. The $0.015$ for the world-frame
scene code alone is arithmetic rather than measurement --- a constant vector has
no variance to regress with, as the row above it states --- and even the $0.07$
is a \emph{linear} probe on features the trunk then processes nonlinearly, so it
does not establish that the trunk cannot recover clearance. What it establishes
is that the quantity is linearly available in one representation and not the
other, which is a claim about the representations rather than about what the
network can compute. The treatment's score is not the separation $d$ in disguise:
...
Sec.~\ref{sec:baselines}'s finding that the world-frame model never exceeds the
...
\textbf{Is the benefit simply letting the encoder see the query?} The probe
above invites a deflationary reading of the whole result. The world-frame scene
code cannot vary with the query \emph{by construction}, so perhaps the reduction
is not buying canonicalisation at all, only query-dependence, which a one-line
change to the encoder would also buy. This is the sharpest objection to the paper
and it is directly testable, so we built the model that separates the two: the
world frame throughout, no change of frame anywhere, with the raw query
concatenated to every obstacle before the pool. Its scene code varies with the
query (within-environment standard deviation $0.022$ against the baseline's
$0.000000$), and it carries $0.07\%$ more parameters.

It converges to $15.59\pm0.38\%$ against the world frame's $14.60\pm0.20$, both
over three seeds. Seed-matched the difference is $+1.01$ points and positive on
every seed ($+1.14$, $+0.50$, $+1.40$), so the effect is real and small.
\emph{Query-dependence alone is worth about one point of the $36.5$}, and
canonicalisation supplies the remaining $35.5$. The deflationary reading is not
supported: a query-dependent scene code is necessary for the mechanism described
above and nowhere near sufficient for the effect.

The level it reaches is worth stating exactly, because it is a cleaner statement
of the paper's floor result than the world-frame model alone provides. At
$15.59\%$ it lands on the straight line's $15.6\%$, not above it: giving the
encoder the query is enough to reach the trivial baseline and not enough to beat
it. What takes the model past the floor at all is the change of frame.
...
is a candidate explanation for the level at which the reduced model plateaus, and
it is separate from the gap: both models share the encoder. We implement an oracle
...
\toprule
Obst. & line & World & Reduct. & Gap & best-of-$20$ \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Both models evaluated across a $5\times$ range of clutter, on freshly
...
costs minutes rather than the days a training sweep would. Both models are trained
...
The finding of Sec.~\ref{sec:baselines} --- that the world-frame model scores what
...
have to argue rather than measure. Thinning the clutter raises both models
...
$24$ obstacles, at which point the two models are no longer distinguishable as
...
a line. The world-frame model gains $0.3$ points across the whole budget range and
never clears its floor at any budget; the reduction clears it and is still
climbing. On the deployment metric the world-frame model moves \emph{backwards},
...
Three things replicate (Table~\ref{tab:se3}). The world-frame model fails to beat
...
models is $2.9\times$ in each: an intermediate rigid-body checkpoint reaches $3.0$
...
the best-validation checkpoints sit at epochs $155$ and $159$, so neither model had
...
survives at a budget where the ``both models are simply undertrained'' reading is
...
observed the world-frame model losing $3.1$ points of deployment performance under
...
spaces agree that additional optimisation makes the world-frame model \emph{worse}
...
two models under a Diffuser/MPD-style DDPM objective~\cite{janner2022diffuser,
...
\emph{both} models between $8$ and $100$ evaluations. Reporting the DDPM model only
...
model's seed-matched $+36.50\pm0.32$ is the informative part.
...
single-core CPU time, while the best learned model reaches $55.8\%$ per sample even
trained to convergence. Part of the shortfall is capacity --- the model is small,
at $2.16$M parameters --- and part is that a $64$-waypoint sampler from an
unconditional prior is a weaker instrument than a complete planner on a problem
with an exact SDF. What it is \emph{not}, any longer, is training budget:
Sec.~\ref{sec:budget} runs both models to a plateau. We draw the comparison anyway,
because a representation result measured only against other configurations of
itself is not anchored to anything.
...
\textbf{Which robots this transfers to, and which it does not.} The reduction
needs the group to act on the \emph{state space}, not merely on the workspace,
and that condition is restrictive enough to state precisely. It holds for the
point mass, whose state is a position in $\mathbb{R}^3$; it holds for the
rigid body of Sec.~\ref{sec:se3}, whose state is a pose in $\SE$, which is why
that domain was built. It fails for a manipulator planning in joint space: the
query still defines a frame in the workspace, but a rigid motion of the
workspace induces no clean action on joint coordinates, so there is no
transformation of the trajectory that the reduction could apply. A
configuration-space planner for an articulated robot therefore inherits none of
this, and we do not claim otherwise. The condition is easy to check for a new
system --- ask whether a rigid motion of the world acts on the state
representation --- and we would rather give a reader that test than a third
domain on which the method happens to work. Mobile bases, free-flying bodies and
end-effector-space planning satisfy it; joint-space planning for arms does not.
...
plausibly reverse it. That is why our answer should not be generalised from this benchmark. It is also
why we do not offer $r$ as the thing to generalise instead: it is cheap and well
defined wherever a group acts, but Sec.~\ref{sec:twosigns} shows it does not
predict what a mechanism is worth, so measuring it elsewhere would not settle the
question our result leaves open.

\textbf{The comparison we previously could not make.} Corollary~\ref{cor:budget}
bounds post-hoc symmetrisation of a given field; a constrained architecture
searches a different hypothesis class and is not covered by it. Earlier versions
of this work were careful not to borrow the bound for that purpose and recorded
the experiment as untested rather than unpromising. Sec.~\ref{sec:arch} now
reports it, and both halves of that caution were justified. The constrained
backbone is worth $+0.15$ points at convergence, so the conclusion the bound
would have suggested happens to hold --- but it is worth a two- to threefold
saving in optimisation, which no reading of $r$ anticipated or could have. A
residual measured on a converged model says nothing about the path taken to
reach it.

Frame averaging also improves endpoint error by roughly $35\%$
($0.0020\!\to\!0.0013$) on every checkpoint tested --- a real effect that does not
propagate to the collision-free rate, because endpoint precision was never the
failure mode.
...
both models train on identical data, but it bears on how additional data should be
...
\emph{both} models ($31.3$ and $30.1$ against $36.0/35.8$ and $35.9/34.8$), so the
mismatch manufactured most of the effect; run at matched seeds it is
$+0.8\pm0.3$. The lesson generalises. The treatment model's across-seed spread is
$1.6$ points, five times the paired spread of the ablation, so any ablation
compared unpaired at this scale is measuring seed noise. Even paired, $n{=}3$
gives two degrees of freedom and a critical $t$ of $4.3$: we can bound these
mechanisms as small but cannot resolve whether they are zero. All four cells of
the main grid now carry three seeds, as does the query-conditioned control of
Sec.~\ref{sec:mechanism}. The local-geometry models of
Table~\ref{tab:localgeom} carry three seeds for the reduction and two for the
world-frame and equivariant models, and the two $250$-environment
local-geometry models carry two each. The remaining $250$-environment cells,
several ablations, and the converged DDPM cells are single seed and are reported
as point estimates.
...
orientations left to the network. In joint-space planning for a $7$-DoF model it
...
coordinates. What survives is narrower and still useful --- for an model on a fixed
...
\textbf{Reproducibility.} The benchmark, the expert pipeline, both learned models,
...
\emph{(2) A third seed on two of the local-geometry cells}: the world-frame and
equivariant models with local geometry carry two seeds against three elsewhere,
though they are also the least seed-sensitive models measured here, with
across-seed standard errors of $0.01$ and $0.02$.
\emph{(3) Convergence at $250$ environments}, so the
...
without any mechanism enforcing it. The benefit is concentrated in the five: all three
mechanisms built for the sixth --- augmentation, frame averaging and an
equivariant architecture --- are worth $+0.8$, $+0.68$ and $+0.15$ points against
$+36.5$, and none is distinguishable from zero at three seeds. Within the five,
...
usually supplies more of them than one expects. We stop there deliberately. We do
not know that it transfers, the scope condition in Sec.~\ref{sec:limits} says
which systems it cannot reach, and Sec.~\ref{sec:localgeom} shows that on this
benchmark a better obstacle encoder is worth more than the frame is.
