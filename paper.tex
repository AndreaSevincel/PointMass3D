...
collision-free on held-out environments --- statistically indistinguishable from
the $15.6\%$ obtained by joining start to goal with a straight line, measured on
the identical problems. Without the reduction the learned model does not beat the
trivial baseline. With it, the same network, data and budget reach $45.6\%$, and,
...
$15.6\%$ of problems, and the world-frame model scores $15.4\%$. The baseline arm
of the comparison --- the configuration a practitioner would reach for by default
--- has learned nothing that beats a straight line. The classical planners
($74$--$100\%$, all under $0.4$\,s on a single CPU core) bound the other side and
show that the learned sampler, at $45.6\%$, is not competitive with them here.
