...
$14.60\pm0.20\%$ to $51.10\pm0.25\%$ across three seeds. The world-frame arm
...
model can remove. We also measure what the residual does \emph{not} do: two arms
...
arm reaches by epoch $40$. Three unrelated mechanisms agreeing that the residual
...
does not do. It does not forecast what a mechanism is worth --- two arms with
...
negative: across three arms of this benchmark $r$ does \emph{not} predict what a
...
rows are the converged $60$-environment arms of Sec.~\ref{sec:budget}, three-seed
...
Every bar is a measured arm rather than a term in a decomposition: the second is
...
goal on the identical problems, which the world-frame arm does not beat.}
\label{fig:cascade}
\end{figure*}
...
points against the arm without that mechanism --- so the rows are not additive,
...
translation-only arms are single seed and are therefore differenced against the
...
$1.56$ points, which governs comparisons between separately trained arms, is
...
with $r$ mattering while quality cancels it. Re-scoring every arm on a common
$15$-degree cone of held-out queries, a subset of all four training cones so that
no arm extrapolates, lifts them all by about $15$ points and confirms that much
of the collapse was distribution shift --- but it does not equalise them: the
spread stays at $12$ points, because an arm trained on a narrow cone generalises
...
reaches by epoch $10$ a level the unconstrained arm needs about $32$ epochs
...
is zero. The same pattern holds at $250$ environments, where the constrained arm
reaches $54.8$ by epoch $80$ against the unconstrained arm's $51.6$.
...
arms are the least seed-sensitive measurements in this work, with the
world-frame arm reproducing to $0.01$ points across its seeds.}
\label{tab:localgeom}
\end{table}
...
reduced arm with local geometry is close to saturating at roughly $80\%$, so the
world-frame arm has room to close on it that the reduced arm no longer has. The
...
\textbf{Trained to a plateau, the world-frame arm does not beat the straight
...
the across-seed standard error --- the right noise model here, since both arms
are trained, run at the same three seeds, and differenced within seed. Both arms
...
\textbf{The world-frame arm does not beat the straight line.} It converges to
...
arms are evaluated on the same $500$ problems and both track per-environment
...
and it is directly testable, so we built the arm that separates the two: the
...
of the paper's floor result than the world-frame arm alone provides. At
...
single-core CPU time, while the best learned arm reaches $55.8\%$ per sample even
...
Sec.~\ref{sec:budget} runs both arms to a plateau. We draw the comparison anyway,
...
\emph{both} arms ($31.3$ and $30.1$ against $36.0/35.8$ and $35.9/34.8$), so the
mismatch manufactured most of the effect; run at matched seeds it is
$+0.8\pm0.3$. The lesson generalises. The treatment arm's across-seed spread is
...
Sec.~\ref{sec:mechanism}. The local-geometry arms of
Table~\ref{tab:localgeom} carry three seeds for the reduction and two for the
world-frame and equivariant arms, and the two $250$-environment
local-geometry arms carry two each. The remaining $250$-environment cells,
...
equivariant arms with local geometry carry two seeds against three elsewhere,
though they are also the least seed-sensitive arms measured here, with
