...
$+39.5$ points against the world frame where the reduction is worth $+35.8$, so
...
with local geometry supplied, the reduction is still worth $+24.7$ points. We
...
Table~\ref{tab:arch} is unambiguous in both directions. The constrained model
...
\bottomrule
\end{tabular}
\caption{Local geometry against the query frame, at convergence. All cells are
seed $0$ so the differences are seed-matched; the three-seed means for the two
left-hand cells in the first two rows are $14.60\pm0.20$ and $51.10\pm0.25$. The
right-hand column is currently a single seed.}
\label{tab:localgeom}
\end{table}
...
is worth $+39.5$ points where the reduction is worth $+35.8$. The
...
local geometry present the reduction is still worth $+24.7$ points. We ran this
cell specifically to find out whether it would overturn our main result, and
report that it did not.

\textbf{The two are sub-additive, and the constraint finally pays.} Treating the
effects as independent predicts $15.0+35.8+39.5=90.3$; the measured value is
$79.2$. Both address the same underlying difficulty --- relating query geometry
to scene geometry --- and neither subsumes the other. Against this, the
equivariant backbone is worth $+0.1$ points without local geometry and $+3.0$
with it. That interaction is what the architecture predicts: the rotating stream
