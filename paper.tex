...
Both arms are flat by the end --- each gains $0.2$ points over the final ten
epochs --- so this is a converged comparison rather than a comparison of
convergence rates. Three things follow.

The world-frame arm converges to $15.0\%$, against the $15.6\%$ straight line of
Sec.~\ref{sec:baselines}. Trained to a plateau with $15$ times the budget, it
remains statistically indistinguishable from joining the endpoints with a line.
From epoch $30$ onwards it moves $1.3$ points in total, wandering between $13.7$
and $15.0$, which is about the evaluation noise. This is the strongest form of
the paper's central negative claim, and at $20$ epochs it was not available: a
reader could always answer ``it is undertrained''.

The gap \emph{widens} monotonically with budget, from $17.9$ to $35.8$ points.
Undertraining was not manufacturing it; if it had been, the flat arm would be the
one with headroom.

And the reduction at $60$ environments and $300$ epochs ($50.8\%$) exceeds the
...
At $250$ environments the same curve is steeper: the gain rises from $+0.22$ at
epoch $10$ to a peak of $+1.57$ at epoch $180$, settling at $+0.9$ over the final
eight snapshots, while $r$ moves only from $0.0163$ to $0.0173$. So the mechanism
...
the variable that tracks the benefit is quality. A clean test would hold quality
fixed and vary $r$, by training the narrow-cone arms until their $K{=}1$ matches
the wide-cone arm; we have not run it.
...
DDPM          & $100$ & $12.27$ & $28.97$ & $\mathbf{+16.71\pm0.42}$ \\
\bottomrule
\end{tabular}
...
The gap replicates (Table~\ref{tab:objective}). It is smaller than under flow
matching --- $+16.7$ against $+20.9$, each at its own best integrator budget ---
and it is not close to zero: at three seeds the DDPM difference is over $30$
standard errors.
...
We note the budget caveat that applies to both rows equally: these are $20$-epoch
models, and Sec.~\ref{sec:budget} shows that budget is well short of convergence,
where the flow gap widens from $20.9$ to $35.8$ points. The DDPM comparison is
matched to the flow arm rather than converged, so $+16.7$ should be read as the
value at a common, inadequate budget, not as the objective's ceiling.
